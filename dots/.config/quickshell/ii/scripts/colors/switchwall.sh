...
is_video() {
    local extension="${1##*.}"
    case "${extension,,}" in
        mp4|webm|mkv|avi|mov|m4v|ogv) return 0 ;;
        *) return 1 ;;
    esac
}

kill_existing_video_wallpaper() {
...
pkill -x mpvpaper || true
EOF
    printf 'exec mpvpaper -p -o %q %q %q\n' "$VIDEO_OPTS" '*' "$video_path" >> "$RESTORE_SCRIPT.tmp"
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            if [ ${#missing_deps[@]} -gt 0 ]; then
                ...
                echo "Arch/AUR: yay -S ${missing_deps[*]}"
                action=$(notify-send \
                    -a "Wallpaper switcher" \
                    -c "im.error" \
            ...
            nohup mpvpaper -p -o "$VIDEO_OPTS" '*' "$video_path" >/dev/null 2>&1 &

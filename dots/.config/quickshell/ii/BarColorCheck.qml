...
ShellRoot {
    ...
    Component.onCompleted: {
        ...
        if (!Qt.colorEqual(Appearance.barForegroundAt(0, null, 1, 0), "#FFFFFF"))
            throw new Error("Media mode foreground is not white");
    ...
    Timer {
        ...
        onTriggered: {
            if (Appearance.wallpaperSampleWidth <= 1) {
                if (++root.attempts >= 50) throw new Error("Wallpaper color sampler timed out");
                return;
            }
            ...
            if (Appearance.barSampleRows <= 1 || Appearance.barBackgroundSamples.length !== expected)
                throw new Error("Wallpaper color sampler is not a 2D grid");
            ...
            if (startPalette.sampleCount !== 5 || endPalette.sampleCount !== 5)
                throw new Error("Bar palette is not using a five-point patch");
            if (startPalette.centerColumn === endPalette.centerColumn)
                throw new Error("Parallax progress did not move the wallpaper sample");
            if (startRect.x === endRect.x || startRect.width <= 0 || startRect.height <= 0)
                throw new Error("Liquid glass wallpaper crop did not follow parallax");
            if (systemMonitorLoader.status !== Loader.Ready || !systemMonitorLoader.item)
                throw new Error("Adaptive system monitor failed to load");
            if (Appearance.colors.transparentBar && !systemMonitorLoader.item.liquidGlassEnabled)
                throw new Error("Bar component did not enable liquid glass");

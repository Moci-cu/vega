...
Item { // Wrapper
    ...
    function focusFirstItem() {
        if (root.timerPanelOpen)
            return;
        if (root.todoPanelOpen) {
            todoPanelLoader.item?.moveSelection(0);
            return;
        }
    ...
    function moveSelection(delta, linear = false) {
        if (root.timerPanelOpen)
            return;
        if (root.todoPanelOpen) {
            todoPanelLoader.item?.moveSelection(delta);
            return;
        }
        if (root.wifiPanelOpen) {
    ...
    Connections {
        ...
        function onTimerPanelRequested() {
            root.wifiPanelOpen = false;
            root.bluetoothPanelOpen = false;
            root.todoPanelOpen = false;
            root.timerPanelOpen = true;
            root.setSearchingText("");
            Qt.callLater(root.focusSearchInput);
        }

        function onTodoPanelRequested() {
            root.wifiPanelOpen = false;
            root.bluetoothPanelOpen = false;
            root.timerPanelOpen = false;
            root.todoPanelOpen = true;
            root.setSearchingText("");
            Qt.callLater(root.focusSearchInput);
        }
    }
    ...
    Item { // Background
        ...
        Item {
            ...
            SearchBar {
                ...
                inputPlaceholder: root.todoPanelOpen ? Translation.tr("Search tasks…")
                    : root.timerPanelOpen ? Translation.tr("Timer")
                    : root.wifiPanelOpen ? Translation.tr("Search Wi-Fi networks…")
        ...
        Item {
            ...
            Item {
                ...
                Loader {
                    ...
                    active: root.todoPanelOpen
                    visible: active
                    anchors.fill: parent
                    sourceComponent: TodoPanel {
                        filterText: root.searchingText
                    }
                }
            }

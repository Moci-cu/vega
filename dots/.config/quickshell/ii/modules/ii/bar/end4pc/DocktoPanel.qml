...
Item {
    ...
    component AppIcon: IconImage {
        ...
        // The shared lookup populates caches; run it outside a source binding.
        function refresh() { source = Quickshell.iconPath(TaskbarApps.getCachedIcon(appId), "image-missing"); }
        onAppIdChanged: Qt.callLater(refresh)
        Component.onCompleted: refresh()
        Connections {
            target: DesktopEntries
            function onApplicationsChanged() { Qt.callLater(icon.refresh); }
        }
        Connections {
            target: TaskbarApps
            function onIconThemeRevisionChanged() { Qt.callLater(icon.refresh); }
        }
    }
    ...
    function _endPinnedItemDrag() {
        ...
        if (JSON.stringify(pinnedApps) !== JSON.stringify(_workOrder)) {
    ...
    function _cancelPinnedDrag() {
        ...
        dragSourceIndex = -1
        _dragTargetIndex = -1
        Qt.callLater(function() { _suppressTranslateAnim = false; suppressClick = false })
    }
        ...
        Flow {
            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    property var    appEntry:     TaskbarApps.apps.find(a => TaskbarApps.normalizeAppId(a.appId) === TaskbarApps.normalizeAppId(appId)) ?? null
                    ...
                    RippleButton {
                        ...
                        onClicked: {
                            if (root.dragging || root.suppressClick) return
                    ...
                    RippleButton {
                        ...
                        contentItem: Item {
                            ...
                            AppIcon {
                                id: activeIcon
                                anchors.centerIn: parent
                                appId: activeSlot.modelData.appId
                                implicitSize: root.iconSize
                            }
